import { IData, IDataOptions, Options, InsertChildType, ITreeNode, InsertSiblingType, RemoveType } from '../types/index'
...
export class TreeView<T> {
  ...
  enhanceNodes(shallow = false) {
    if (shallow) {
      this.enhancedData = this._staticShallowEnhance(this.enhancedData)
    }
    this.traverse(node => {
  ...
  private _enhance(tree: any[], options: Options, parentNode?: TreeNode): TreeNode[] {
    ...
    this.sort(tree).forEach((child: IData, index: number) => {
      ...
      newChild.options.reactKey = `${(newChild as unknown as Record<string, TreeNode>)[this.options.idKey]}-${newChild.getPath()}-${index}`
  ...
  private _staticShallowEnhance(tree: TreeNode[], parentNode?: TreeNode): TreeNode[] {
    const result: TreeNode[] = []
    tree.forEach((child: TreeNode) => {
      const currentChildren = (child as unknown as Record<string, TreeNode[]>)[this.options.childrenKey] || []
      child.options.parent = parentNode
      child.setPath(parentNode ? parentNode.options.path : '')

      let children: TreeNode[] = []
      if (currentChildren.length !== 0) {
        children = this._staticShallowEnhance(currentChildren, child)
      }
  ...
  private _calculateChildrenCount(child: TreeNode) {
    ...
    if (lastChild && lastChild.isOpened()) {
      ...
      child.options.currentChildrenCount = globalCount - lastChild.options.childrenCount
    } else {
      child.options.currentChildrenCount = child.options.childrenCount
    }
  }
